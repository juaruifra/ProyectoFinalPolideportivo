...
namespace ProyectoFinal.view_new
{
    public partial class VistaSocios : UserControl
    {
        ...
        private void BtnBorrarSeleccionado_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                var confirmar = ModalMessage.ShowModal(
                    $"¿Desea borrar el socio '{socio.Nombre} {socio.Apellidos}'? Esta acción no se puede deshacer.",
                    "Socios",
                    3);
                ...
                if (_controller.Borrar(socio, ref tituloError, ref mensajeError))
                {
                    // Recargamos.
                    LimpiarFormulario();
                    CargarDatos();
                    ModalMessage.ShowModal("Socio borrado correctamente.", "Socios", 2);
                    return;
                }
        ...
        {
            btnEditarSeleccionado.IsEnabled = dgSocios.SelectedItem != null; // Editar.
            if (btnBorrarSeleccionado != null) btnBorrarSeleccionado.IsEnabled = dgSocios.SelectedItem != null; // Borrar.
        }
    }
}
